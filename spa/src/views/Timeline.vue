          ...
          >
            <v-row class="pa-0" v-if="!drawer">
              <v-col cols="4" class="pa-0">
                <small
                  :class="
                    selectedChunk == chunk ? 'amber--text' : 'white--text'
                  "
                  >{{ dtShorten(chunk.dt) }}</small
                >
              </v-col>
              <v-col class="pa-0">
                <span
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <v-timeline-item
                ...
                :fill-dot="!selectedChunk == chunk"
                @click.native="callSelectChunk($event, chunk)"
                @mouseover.native.stop="tooltipShown = i"
                @mouseleave.native.stop="tooltipShown = null"
              >
                <v-row class="pa-0" v-if="!drawer">
                  <v-col cols="4" class="pa-0">
                    <small
...
export default {
  ...
  methods: {
    ...
    titleShorten(title) {
      return (title.length > 27) ? title.substr(0, 24).concat('...') : title;
    },
    getVisibleChunks() {
      if (!this.search) return this.selectedFile.chunks;

      return this.selectedFile.chunks.filter(
        x =>
          this.dtShorten(x.dt).includes(this.search) ||
